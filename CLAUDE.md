...
- **Google OAuth (no server)**: Users link Google accounts via a static GitHub Pages callback page. Tokens stored per-user in `data/google/<number>.json`. Google tools use `sender_jid` (not `user_jid`) so group members access their own accounts. Token auto-refresh via httpx calls to `oauth2.googleapis.com/token`.
